...
model = pytorch_model(config="pi05_libero", checkpoint_dir="./model_weights/pi05_pc_libero_torch")
...
def save_model():
    """Health check endpoint."""
    model.save_model("./model_weights/pi05_pc_libero_torch/model.safetensors")
    return jsonify({"status": "model saved", "model": "pi05_pytorch"}), 200
...
def run_performance():
    try:
        ...
        for _ in tqdm(range(time_step_num), desc="run model performance"):
            ...
            example['observation/image'] = torch.from_numpy(example['observation/image'])
            example['observation/wrist_image'] = torch.from_numpy(example['observation/wrist_image'])
        ...
        return jsonify({
            'status': 'error',
            'error': str(e),
            'traceback': error_msg  # Include for debugging
        }), 500
